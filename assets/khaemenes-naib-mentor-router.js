(function attachKhaemenesNAIB(global){
  ...
  function normalizeAgeBand(value){
    const age=clean(value,30).toLowerCase();
    return ["2-3","3-5","preschool","kindergarten","elementary"].includes(age) ? age : "";
  }
  ...
  function presentationFor({stage,ageBand}={}){
    ...
    if(normalizedStage==="preschool"){
      return age==="2-3" ? PRESENTATIONS.nestling : PRESENTATIONS.storybook;
    }
  ...
  function assignmentToken(stage){
    try{
      if(global.crypto?.randomUUID) return `naib:${stage}:${global.crypto.randomUUID()}`;
      if(global.crypto?.getRandomValues){
        const bytes=new Uint32Array(2);
        global.crypto.getRandomValues(bytes);
        return `naib:${stage}:${bytes[0].toString(36)}${bytes[1].toString(36)}`;
      }
    }catch{}
    return `naib:${stage}:local`;
  }
  ...
  function buildArchaemenesAssignment(context){
    ...
    return Object.freeze({
      ...
      assignmentId:assignmentToken(stage),
      assignedBy:"NAIB",
      assignmentAuthority:"naib-mentor-router",
      assignmentMode:"local-transition-policy",
  ...
  function assignMentor(context={}){
    const safeContext={
      ...
      intent:clean(context.intent,80),
      interests:cleanList(context.interests)
    };

    if(YOUNG_LEARNER_STAGES.has(safeContext.stage)){
      return buildArchaemenesAssignment(safeContext);
    }
    ...
    return Object.freeze({
      ...
      reason:"No transitional mentor assignment policy is published for this stage yet.",
      mentor:null
    });
  }
