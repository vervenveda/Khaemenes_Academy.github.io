(function attachKhaemenesNAIB(global){
  ...
  const PRESENTATIONS=Object.freeze({
    ...
    middleScholar:Object.freeze({
      mode:"middle-scholar",
      colors:Object.freeze(["#35596f","#9aa86f"]),
      intro:"I am Archaemenes. I can help you understand the idea, diagnose where the reasoning changed, and plan the next useful step without doing scored work for you."
    }),
    seniorScholar:Object.freeze({
      mode:"senior-scholar",
      colors:Object.freeze(["#243d33","#b48b45"]),
      intro:"I am Archaemenes, Scholar and Educational Mentor of Khaemenes Academy. I can help with concepts, evidence, research, planning, and rigorous reasoning while your course remains the authority for mastery and progression."
    }),
    higherScholar:Object.freeze({
      mode:"higher-scholar",
      colors:Object.freeze(["#1f2937","#b48b45"]),
      intro:"I am Archaemenes. I can support advanced study, proofs, research, synthesis, and independent scholarship while formal course requirements remain authoritative."
    }),
    faculty:Object.freeze({
      mode:"faculty-advisory",
      colors:Object.freeze(["#403b31","#d9d3c7"]),
      intro:"I am Archaemenes, Scholar and Educational Mentor of Khaemenes Academy. I can help review evidence, curriculum, mastery signals, and learner support while formal academic decisions remain with the responsible teacher, course, or Academy authority."
    })
  });
  ...
  function normalizeStage(value){
    ...
    if(["higher","higher-learning","college","university","adult-learning","adult","continuing-learning","lifelong-learning"].includes(stage))return"higher";
  ...
  function presentationFor({stage,ageBand,role,surface}={}){
    ...
    const age=clean(ageBand,30).toLowerCase();
    ...
    if(stage==="middle")return PRESENTATIONS.middleScholar;
    if(stage==="high")return PRESENTATIONS.seniorScholar;
    if(stage==="higher")return PRESENTATIONS.higherScholar;
    return null;
  }
  ...
  function mentorEnvelope(context,presentation,mode){
    return Object.freeze({
      ...
      specialistDomain:ARCHAEMENES.specialistDomain,
      principles:[...ARCHAEMENES.principles],
      assignedBy:"NAIB",
      assignmentMode:mode,
  ...
  function buildArchaemenesAssignment(context){
    ...
    const presentation=presentationFor({...context,stage})||PRESENTATIONS.storybook;
    return Object.freeze({
      status:"assigned",
      contractVersion:VERSION,
      assignmentId:`naib:${clean(context.personId||context.learnerId||"local",80)}:${stage}`,
      assignedBy:"NAIB",
      assignmentAuthority:"naib-mentor-router",
      assignmentMode:"lifelong-local-policy",
      mentorId:ARCHAEMENES.id,
      specialist:ARCHAEMENES.name,
      stage,
      grade:normalizeGrade(context.grade),
      ageBand:clean(context.ageBand,30),
      surface:clean(context.surface||"unknown",100),
      subject:clean(context.subject||context.domain||"",80),
      courseId:clean(context.courseId||"",100),
      intent:clean(context.intent||"mentor-guidance",80),
      interests:list(context.interests,30),
      mentor:mentorEnvelope(context,presentation,"lifelong-local-policy")
    });
  }
  ...
  function routeEducator(context={}){
    ...
    return Object.freeze({
      ...
      assignedBy:"NAIB",
      assignmentAuthority:"naib-mentor-router",
      assignmentMode:"teacher-advisory-routing",
      destination:"Khaemenes Academy Teacher Administration",
      destinationPath:"/Khaemenes_Academy.github.io/teacher-admin/",
      role:"educator",
      stage:normalizeStage(context.stage||"academy-wide"),
      surface:clean(context.surface||"teacher-admin",100),
  ...
  function routeLearnerEntry(context={}){
    ...
    return Object.freeze({
      ...
      assignedBy:"NAIB",
      assignmentAuthority:"naib-mentor-router",
      assignmentMode:"learner-entry-routing",
      learnerId:clean(context.learnerId||context.personId||"",100),
      stage,
      grade,
      destination:url,
      destinationLabel:destination?.label||grade||stage,
      requiresManualStageChoice:!stage||stage==="unknown",
  ...
  function assignMentor(context={}){
    ...
    const stage=normalizeStage(context.stage);
    if(LEARNER_STAGES.has(stage))return buildArchaemenesAssignment({...context,stage});
    return Object.freeze({
      status:"unassigned",
      contractVersion:VERSION,
      assignedBy:"NAIB",
      ...
      assignmentMode:"lifelong-local-policy",
      stage,
      reason:"No learner mentor assignment policy is published for this stage.",
      mentor:null
    });
  }
  ...
  const API=Object.freeze({
    ...
    currentPolicy:Object.freeze({
      ...
      high:"archaemenes",
      higher:"archaemenes",
      educator:"teacher-admin→archaemenes"
    }),
    authority:Object.freeze({
